...
**trialwatch** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
the suite's integration SDK. It maps any tool's JSON into a canonical **Finding** and
forwards it to the platforms that fit the **Private-AI backbone** domain.
...
Once `trialwatch` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
trialwatch ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
trialwatch ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `trialwatch` outputs:

```python
from cognis_connect import normalize, edgemesh
findings = [normalize(rec, source="trialwatch") for rec in records]   # records = your JSON output
