...
How **trialwatch** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `trialwatch` lives in the **Private-AI backbone** cluster.
...
| from | relation | to |
|---|---|---|
| `trialwatch` | JSON findings compose with | [`edgemesh`](https://github.com/cognis-digital/edgemesh), [`modelroute`](https://github.com/cognis-digital/modelroute), [`uncensored-fleet`](https://github.com/cognis-digital/uncensored-fleet), [`cognis-code`](https://github.com/cognis-digital/cognis-code) |
| `trialwatch` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `trialwatch` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `trialwatch` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the private-ai backbone cluster.** `trialwatch` output feeds the next tool:
```bash
trialwatch ... --format json > out.json          # this tool's findings (see `trialwatch --help`)
edgemesh ... < out.json                      # the cluster sibling consumes them
```
...
trialwatch ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
trialwatch ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
trialwatch ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `trialwatch`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `trialwatch` slots
into the **Private-AI backbone** stack below.
