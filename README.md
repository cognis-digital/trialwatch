...
[![PyPI](https://img.shields.io/pypi/v/cognis-trialwatch.svg)](https://pypi.org/project/cognis-trialwatch/)
[![CI](https://github.com/cognis-digital/trialwatch/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/trialwatch/actions)
...
TRIALWATCH exists for one job — query, diff, and monitor clinicaltrials.gov records, alerting on status, enrollment, or result changes. — and does it without a SaaS bill or heavyweight setup.
...
```bash
trialwatch --version
trialwatch scan .                      # scan the current project
trialwatch scan . --format json
trialwatch scan . --fail-on high       # non-zero exit for CI gates
trialwatch mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **ClinicalTrials.gov API + watchman**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`trialwatch` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
