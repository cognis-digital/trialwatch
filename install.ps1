# Comprehensive installer for cognis-digital/trialwatch (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# trialwatch is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "trialwatch"
$Url  = "git+https://github.com/cognis-digital/trialwatch.git"
$Git  = "https://github.com/cognis-digital/trialwatch.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: trialwatch"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: trialwatch"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: trialwatch"; exit 0 }
}
